...
def create_test_train_split(dataset_name: str, train_test_split: float = 0.8) -> None:
    ...
    dataset_train.to_csv(
        dataset_path.replace(f"all_{dataset_name}", f"train_{dataset_name}"),
        index=False,
    )
    dataset_test.to_csv(
...
def parse_arguments() -> argparse.Namespace:
    ...
    # Parse arguments
    parser.add_argument(
        "-d", "--dataset", help="Name of dataset/project", required=True
    )
...
if __name__ == "__main__":
    ...
    create_test_train_split(args.dataset)
