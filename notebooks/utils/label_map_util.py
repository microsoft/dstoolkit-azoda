...
def _validate_label_map(label_map):
    ...
    for item in label_map.item:
        ...
        if (item.id == 0 and item.name != 'background' and
            ...
            raise ValueError('Label map id 0 is reserved\
                                for the background label')
...
def convert_label_map_to_categories(label_map,
    ...
    Args:
        ...
        max_num_classes: maximum number of (consecutive) label
        indices to include.
        use_display_name: (boolean) choose whether to load 'display_name'
        field as category name.
        If False or if the display_name field does not exist,
        uses 'name' field as category names instead.

    Returns:
        categories: a list of dictionaries representing
...
def get_label_map_dict(label_map_path,
    ...
    Args:
        ...
        use_display_name: whether to use the label map items'
        display names as keys.
        fill_in_gaps_and_background: whether to fill in gaps and
        background with
        respect to the id field in the proto. The id: 0 is reserved for the
        'background' class and will be added if it is missing. All other
        missing
    ...
    Raises:
        ValueError: if fill_in_gaps_and_background and label_map
        has non-integer or
        negative values.
    """
    ...
    if fill_in_gaps_and_background:
        ...
        if not all(isinstance(value, int) for value in values):
            raise ValueError('The values in label map must be integers'
                             'in order to fill_in_gaps_and_background.')
        ...
        if len(values) != max(values) + 1:
            ...
            for value in range(1, max(values)):
                if value not in values:
                    # TODO(rathodv): Add a prefix 'class_' here once the tool
                    # to generate teacher annotation adds
...
def create_categories_from_labelmap(label_map_path, use_display_name=True):
    ...
    use_display_name: (boolean) choose whether to load 'display_name' field
        as category name.  If False or if the display_name field does not
        exist, uses 'name' field as category names instead.
